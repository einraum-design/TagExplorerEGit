		...
		if (msql2.connect()) {
			...
			if (now.getTime() - p5.getNewestDate(file).getTime() > 5000) {
				...
				if (p5.drawAccessShapes) {
					file.setShape(p5.generateAccessShape(file));
...
//				}
			...
			Tag_App tag = new Tag_App(tableName, msql.getInt("ID"), msql.getString("name"), msql.getString("image"),
					msql.getInt("count"));
			String url = msql.getString("url");
			if(url != null){
				tag.setUrl(url);
			}
			t = tag;
